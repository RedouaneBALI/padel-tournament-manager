...
export default function MatchResultCard({ teamA, teamB, editable = false, gameId, score, tournamentId, onScoreSaved, winnerSide, stage }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="text-center text-sm text-muted-foreground border-t border-border px-4 py-2 space-y-1">
        <div className="font-medium">
          {stage ? <span>{stage}</span> : <span className="italic text-gray-400">Aucun détail défini</span>}
        </div>
      </div>
    </div>
  );
